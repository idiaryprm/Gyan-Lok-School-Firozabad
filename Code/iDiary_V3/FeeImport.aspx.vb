...
Public Class ImportMarks
    ...
    Protected Sub btnCheckExcel_Click(sender As Object, e As EventArgs) Handles btnCheckExcel.Click
        ...
        While (1)
            i += 1
            If oSheet.Range("A" & i).Value = Nothing Then
                Exit While
            End If
        End While
        ...
        For row = 2 To RowNo + 1
            Dim DepositDate As String = Trim(oSheet.Range("C" & row).Value)
            Dim RegNo As String = Trim(oSheet.Range("D" & row).Value)
            Dim Amount As Double = 0
            Dim Flag As Boolean = True
            Try
                Flag = False
                Amount = Trim(oSheet.Range("G" & row).Value)
            Catch ex As Exception
                Flag = True
            End Try
            ...
            If ValidateDetail(RegNo) <> "" Or Flag = True Then
                AddTmpFeeReject(Trim(oSheet.Range("A" & row).Value), Trim(oSheet.Range("C" & row).Value), Trim(oSheet.Range("D" & row).Value) & "-" & Trim(oSheet.Range("E" & row).Value), Trim(oSheet.Range("G" & row).Value))
    ...
    Private Sub AddTmpFeeReject(SNo As String, Dated As String, Details As String, Amount As String)
        ...
        Dim sqlStr As String = "Insert into tmpFeeReject values ('" & SNo & "','" & Dated & "','" & Details & "','" & Amount & "',1)"
    ...
    Private Function GetSID(RegNo As String) As String
        ...
        Dim sqlStr As String = "Select SID,ClassID,SecID,SName,FeeGroupID From vw_Student Where RegNo='" & RegNo & "' AND ASID=" & Request.Cookies("ASID").Value
        ...
        While myReader.Read
            ...
            SecID = myReader(2)
            SName = myReader(3)
            FeeGroupID = myReader(4)
        End While
        myReader.Close()
        
        
        Return SID & "/" & ClassID & "/" & SecID & "/" & SName & "/" & FeeGroupID
    End Function
    Private Sub ImportFee(SNo As String, ChallanNo As String, DepositDate As String, RegNo As String, TermNo As String, ExcelAmount As Double, PaymentMode As String, ChequeNo As String, ChqDate As String, Details As String)
        ...
        Dim FeeGroupID As String = TempRv.Split("/")(4)
        Dim PaymentModeID = FindMasterID(12, PaymentMode)
        ...
        If SID > 0 Then
            ...
            sqlStr = "Select TermID from TermMaster where TermNo='" & TermNo & "' and FeeGroupID=" & FeeGroupID
            ...
            sqlStr = "Insert into FeeDeposit(ASID, SID, TermID, DepositDate, DepositMode, DepositDetails, isDeposit, BranchID) Values(" & _
                    Request.Cookies("ASID").Value & "," & SID & "," & TermID & "," & _
                  "'" & DepositDate & "'," & _
                    PaymentModeID & "," & "'" & Details & "',1,'')"
            ...
            If ChallanNo <> "" Then
                sqlStr = "Select * from vw_FeeChallanIndivisual where FeeChallanAmount>0 and FeeChallanID ='" & ChallanNo & "'"
                Dim myCount As Integer = 0
                
                
                Dim myReader As SqlDataReader = ExecuteQuery_ExecuteReader(sqlStr)
                Dim FeeTypeID As String = ""
                Dim FeeAmount As String = ""
                While myReader.Read
                    FeeTypeID = myReader("FeeTypeID")
                    'tdx1.Text = myReader("FeeTypeName")
                    FeeAmount = myReader("FeeChallanAmount")
                    InsertFeeChallanDetail(FeeDepositID, FeeTypeID, FeeAmount, PaymentModeID, ChequeNo)
                    ImportFlag = True
                End While
            Else
                sqlStr = "Select FeeTypeID,FeeChallanAmount from vw_FeeChallanIndivisual where TermID ='" & TermID & "' and SID=" & SID
                ' Sum(FeeChallanAmount)=" & SubmitAmount & " and 
                Dim myCount As Integer = 0
                ...
                End While
            End If
            If ImportFlag = True Then
                Save_Log(PaymentModeID, RegNo, SName, SubmitAmount, ChequeNo, "INSERT")
            Else
                AddTmpFeeReject(SNo, ChangeDate(DepositDate), Details, ExcelAmount)
            End If
        Else
            AddTmpFeeReject(SNo, ChangeDate(DepositDate), Details, ExcelAmount)
        End If
    ...
    End Sub
    Private Sub InsertFeeChallanDetail(FeeDepositeID As String, FeeTypeID As String, FeeAmount As String, PaymentModeID As String, ChequeNo As String)
       
       
       
        
        Dim sqlStr As String = "Insert into FeeDepositDetails Values(" & _
           FeeDepositeID & "," & _
           FeeTypeID & "," & _
           FeeAmount & "," & _
          FeeAmount & ",0)"
    ...
    Private Sub Save_Log(Mode As Integer, RegNo As String, Sname As String, Amount As Double, ChequeNo As String, ByVal type As String)
        ...
        If (type.Contains("INSERT") = True) Then
            If Mode = 1 Then
                log1 = "Import Fee of RegNo : " & RegNo & ", Name : " & Sname & ", Fee Sum : " & Amount & ", Payment Mode: Cash, Date : " & Dated
            Else
                log1 = "Import FeeRegNo : " & RegNo & ", Name : " & Sname & ", Fee Sum : " & Amount & ", Cheque NO : " & ChequeNo & ", Date : " & Dated
            End If
        End If
    ...
    Public Function CheckFeeImport(SID As Integer, depositDate As String) As Boolean
        ...
        'Dim sqlStr As String = ""
        Dim sqlStr As String = "Select Count(*) from FeeDeposit  where SID='" & SID & "' and DepositDate ='" & depositDate & "'"
    ...
    Protected Sub btnImportMarks_Click(sender As Object, e As EventArgs) Handles btnImportFee.Click
        ...
        For row = 2 To RowNo
            ...
            Dim ChallanNo As String = Trim(oSheet.Range("B" & row).Value)
            Dim DepositDate As String = ChangeDate(Trim(oSheet.Range("C" & row).Value))
            Dim RegNo As String = Trim(oSheet.Range("D" & row).Value)
            Dim TermNo As String = Trim(oSheet.Range("F" & row).Value)
            Dim DepositMode As String = Trim(oSheet.Range("H" & row).Value)
            Dim ChequeNo As String = Trim(oSheet.Range("I" & row).Value)
            Dim ChequeDate As String = Trim(oSheet.Range("J" & row).Value)
            Dim DESC As String = Trim(oSheet.Range("K" & row).Value)
            Dim Amount As Double = 0
            Dim Flag As Boolean = True
            Try
                Flag = False
            ...
            ImportFee(SNo, ChallanNo, DepositDate, RegNo, TermNo, Amount, DepositMode, ChequeNo, ChequeDate, DESC)
        Next
        GridView1.DataSource = SqlDataSource1
        GridView1.DataBind()
        InitControls()
